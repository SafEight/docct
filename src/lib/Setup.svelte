...
<div class="flex flex-col justify-center grow gap-6 pb-24 md:gap-18 md:pb-0">
  <div class="flex flex-col md:flex-row gap-6 md:gap-18 items-center grow">
    <div class="flex flex-col gap-6 md:gap-9">
          ...
          <div class="flex flex-col md:flex-row items-center md:items-center gap-4">
            ...
            {#if state.settings.adaptationMode === 'classic'}
              <div class="flex items-center gap-2">
                <button type="button" class="cursor-pointer bg-[#a9b4cc] p-2 rounded-md w-11 h-11 flex items-center justify-center font-bold text-[#0f121a] text-lg" onmousedown={(e) => e.preventDefault()} onclick={() => handleAdaptationStep(state.settings.adaptationStepMs - 50)} aria-label="Decrease step size">
                  <span>−</span>
                </button>
                ...
                <button type="button" class="cursor-pointer bg-[#a9b4cc] p-2 rounded-md w-11 h-11 flex items-center justify-center font-bold text-[#0f121a] text-lg" onmousedown={(e) => e.preventDefault()} onclick={() => handleAdaptationStep(state.settings.adaptationStepMs + 50)} aria-label="Increase step size">
                  <span>+</span>
                </button>
      ...
      <div class="flex flex-col md:flex-row gap-6 md:gap-9">
        <!-- Duration -->
        <div class="flex flex-col items-start gap-4">
          ...
          <div class="flex overflow-hidden rounded-xl">
            ...
            <div class="flex gap-2 bg-[#0f121a] p-2 pl-0">
              {#each [5, 10, 15, 30, 60] as preset}
                <button type="button" aria-label={`Set duration to ${preset} minutes`} class="flex min-h-11 min-w-8 cursor-pointer items-center justify-center rounded-md bg-[#a9b4cc] p-2" onmousedown={(e) => e.preventDefault()} onclick={() => handleDuration(preset)}>
          ...
          <div class="flex overflow-hidden rounded-xl">
            ...
            <div class="flex gap-2 bg-[#0f121a] p-2 pl-0">
              {#each [0.5, 1, 2, 3, 5] as preset}
                <button type="button" aria-label={`Set interval to ${preset} seconds`} class="flex min-h-11 min-w-8 cursor-pointer items-center justify-center rounded-md bg-[#a9b4cc] p-2" onmousedown={(e) => e.preventDefault()} onclick={() => handleStartingInterval(preset)}>
                  <span class="text-[#0f121a] text-sm font-bold">{preset}</span>
                </button>
              {/each}
            </div>
        ...
        {#if state.settings.intervalMode === 'adaptive'}
          ...
          <div class="flex flex-col items-start gap-4">
            ...
            <div class="flex overflow-hidden rounded-xl">
              ...
              <div class="flex gap-2 bg-[#0f121a] p-2 pl-0">
                {#each [0.5, 1, 2, 3, 5] as preset}
                  <button type="button" aria-label={`Set minimum interval to ${preset} seconds`} class="flex min-h-11 min-w-8 cursor-pointer items-center justify-center rounded-md bg-[#a9b4cc] p-2" onmousedown={(e) => e.preventDefault()} onclick={() => handleMinimumInterval(preset)}>
                    <span class="text-[#0f121a] text-sm font-bold">{preset}</span>
                  </button>
                {/each}
              </div>
  ...
  <!-- Mobile primary action remains visible while the longer setup form scrolls. -->
  <button aria-label="Start session" class="fixed bottom-4 left-1/2 z-10 flex min-h-11 -translate-x-1/2 cursor-pointer items-center justify-center gap-3 whitespace-nowrap rounded-full bg-[#10b981] px-10 py-3 shadow-2xl hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white md:hidden" onclick={startSession}>
    ...
    <span class="text-lg font-semibold text-[#090a0d]">Start session</span>
  </button>

  <!-- Bottom section -->
  <div class="flex flex-col items-center gap-4 pb-12 md:pb-0">
    <button aria-label="Start session" class="hidden cursor-pointer items-center justify-center gap-3 rounded-full bg-[#10b981] px-16 py-5 hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white md:flex" onclick={startSession}>
      <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" class="h-5 w-5 text-[#090a0d]"><path d="M8 5v14l11-7z"/></svg>
    ...
    <div class="flex flex-wrap justify-center gap-x-6 gap-y-1 pt-4 text-sm">
      <a href="https://discord.com/invite/brain" target="_blank" rel="noreferrer" class="group flex min-h-11 flex-wrap items-center justify-center gap-x-2 text-[#7e889c] hover:text-[#a9b4cc]">
        <span>Mindbuilding Discord:</span>
        <span class="text-[#a9b4cc] group-hover:text-[#ffffff]">discord.gg/brain</span>
      </a>
      <a href="https://github.com/SafEight/docct" target="_blank" rel="noreferrer" class="group flex min-h-11 items-center justify-center gap-x-2 text-[#7e889c] hover:text-[#a9b4cc]">
        <span>GitHub:</span>
